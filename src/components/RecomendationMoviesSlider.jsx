...
import ArrowButton from '../pages/home/_partials/ArrowButton';
import { genreMovies, getRecomendations } from '../services/api';
import CardMovies from './CardMovies';

const RecomendationsMoviesSlider = () => {
  const settings = {
  ...
  useEffect(() => {
    getRecomendations(id).then((data) => {
  ...
  return (
    <>
      <div className="mx-[-6px]">
        <Slider {...settings}>
          {recomendationMovies.map((movie, index) => (
            <div
              key={index}
              className=""
            >
...
export default RecomendationsMoviesSlider;
